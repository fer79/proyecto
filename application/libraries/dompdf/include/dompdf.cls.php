...
class DOMPDF {
	...
	/**
	 * Return the underlying Canvas instance (e.g.
	 * CPDF_Adapter, GD_Adapter)
	 *
	 * @return Canvas
	 */
	function get_canvas() {
...
mb_strtolower ( $tag->getAttribute ( "type" ) ) === "text/css") {
	 ...
	 *
	 * @param string $size
	 *        	'letter', 'legal', 'A4', etc. {@link CPDF_Adapter::$PAPER_SIZES}
	...
	function render() {
		...
		if (is_array ( $base_page_style->size )) {
			$this->set_paper ( array (
					0,
					0,
					$base_page_style->size [0],
					$base_page_style->size [1] 
			) );
		}
