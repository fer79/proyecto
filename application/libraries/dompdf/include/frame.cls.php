...
class Frame {
	...
	/**
	 * This frame's original style.
	 * Needed for cases where frames are
	 * split across pages.
	 *
	 * @var Style
	 */
	...
	/**
	 * This frame's first child.
	 * All children are handled as a
	 * doubly-linked list.
	 *
	 * @var Frame
	 */
	...
	protected function ws_keep() {
		...
		return in_array ( $whitespace, array (
				"pre",
				"pre-wrap",
				"pre-line" 
		) );
	 ...
	 *
	 * @param $i string
	 *        	The key of the wanted containing block's dimension (x, y, x, h)
	 *        	
	 * @return float[]|float
	 */
	...
	/**
	 * Return the height of the margin box of the frame, in pt.
	 * Meaningless
	...
	/**
	 * Return the width of the margin box of the frame, in pt.
	 * Meaningless
	...
	function get_padding_box() {
		...
		$x = $this->_position ["x"] + $style->length_in_pt ( array (
				$style->margin_left,
				$style->border_left_width 
		), $cb ["w"] );
		
		$y = $this->_position ["y"] + $style->length_in_pt ( array (
				$style->margin_top,
				$style->border_top_width 
		), $cb ["h"] );
	...
	function is_pre() {
		...
		return $this->_is_cache ["pre"] = in_array ( $white_space, array (
				"pre",
				"pre-wrap" 
		) );
	}
