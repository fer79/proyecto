...
;
	...
	/**
	 * List of all positionned types.
	 * Should really be a constant.
	 *
	 * @var array
	 */
	static $POSITIONNED_TYPES = array (
			"relative",
			"absolute",
			"fixed" 
	);
	...
	/**
	 * List of all table types.
	 * Should really be a constant.
	 *
	 * @var array;
	 */
	static $TABLE_TYPES = array (
			"table",
			"inline-table" 
	);
	 ...
	 * Should also really be a constant.
	 *
	 * @var array
	 */
	...
	/**
	 * Font size of parent element in document tree.
	 * Used for relative font
	 * size resolution.
	 *
	 * @var float
	 */
	 ...
	 *
	 * @param float|array $length
	 *        	the length or array of lengths to resolve
	 ...
	 * properties directly.
	 * Typically __get() is not called directly outside
	 * of this class.
	 * On each modification clear cache to return accurate setting.
	 * Also affects direct settings not using __set
	 *
				...
				if (preg_match ( "/$name\s*$pattern/i", $t, $matches )) {
					...
					switch ($name) {
						...
						case "translateX" :
							$name = "translate";
							$values = array (
									$this->length_in_pt ( $values [0], $this->width ),
									0 
							);
							break;
						...
						case "translateY" :
							...
							$values = array (
									0,
									$this->length_in_pt ( $values [0], $this->height ) 
							);
							break;
	...
	function __toString() {
		return print_r ( array_merge ( array (
