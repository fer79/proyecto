...
function borrar(id){
	...
	if(deleteyes) {
		...
		$.ajax({
			type: "POST",
			url: site_url+"admin_categorias_ajx/borrar",
			data: {'id':id},
...
function listarCategorias(start,search){
	
	search = typeof search !== 'undefined' ? search : '';

	xhrlistar = $.ajax({
		url:	site_url+'admin_categorias_ajx/listar',
		type: 	'POST',
		...
		success: function(result){ 
			if(result=='not_logged'){
				...
				$.each(data, function(i,val) {
					...
					$d = $('<tr id="id-'+data[i].id+'">\
								...
								<td class="align-right">\
									<a href="'+site_url+'adminpanel/categorias/modificar/'+data[i].id+'"><i class="icon-pencil"></i></a>\
				...
				$('.search').keyup(function(){
					...
					xhrbuscar = $.ajax({
						url:	site_url+'admin_categorias_ajx/obtenerTotal', 
						type: 	'POST',
						data: {'search':search},
						success: function(result){ 
							...
							onChange: function(cantidad){
								listarCategorias(cantidad,search);
	...
	<div id="pad-wrapper" class="users-list">
		<div class="row header">
			...
			<div class="col-md-10 col-sm-12 col-xs-12 pull-right">
				...
				<a href="<?php echo base_url();?>adminpanel/categorias/crear"
				...
				<table id="listado" class="table table-hover">
					<thead>
						<tr>
							...
							<th class="col-md-1 sortable">Padre</th>
