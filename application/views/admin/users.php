	...
	function reenviar_validacion($email = '') {
		...
		if ($retorno = $this->auth->reenviar_validacion ( $email )) {
			...
			if ($retorno == 'activo') {
				...
				$data->error = '<div class="alert alert-success">E-mail de enviado. Revisa tu casilla de correo, recibirás las instrucciones para validar tu cuenta. <a href="' . base_url () . 'usuario/revalidar/' . $email . '">Enviar E-mail nuevamente</a></div>';
			}
		}
