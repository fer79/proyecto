...
//     	});
 ...
 $('#colderecha,#colizquierda').wysihtml5();
    ...
    if (emails == null)
      ...
      $.ajax({
        url: site_url+'formularios_ajx/modificar',
        type: 'POST',
        dataType: "json",
        data: {'id':'<?php echo $idFormulario; ?>','publicado':publicado,'titulo':titulo,'abonar':abonar,'lugarabono':lugarabono,'monedacostocurso':monedacostocurso,'costocurso':costocurso,'fechaabonoinicio':fechaabonoinicio,'fechaabonofin':fechaabonofin,'vincular':vincular,'fechainicio':fechainicio,'fechafin':fechafin,'colderecha':colderecha,'colizquierda':colizquierda,'cantidad':cantidad,'tipoformulario':tipoformulario,'campos':JSON.stringify(campos),'emails':JSON.stringify(emails),'categoria':categoria,'fechacomienzocurso':fechacomienzocurso,'cargahoraria':cargahoraria},
        success: function(result){
          ...
          if(result=='not_logged'){
            ...
            $('#formulario1').prepend('<div class="alert alertgeneral alert-success">\
              <i class="icon-ok-sign"></i> Formulario Modificado!.\
            </div>').fadeIn('slow');
			...
			<form name="formulario1" id="formulario1">
				<div class="column">
					...
					<div class="form-group">
						...
						<div class="col-md-7">
							<input name="titulo" value="<?php  echo $ret['titulo']; ?>"
								id="titulo" class="form-control" type="text">
						</div>
					</div>
						...
						<div class="ui-select">
							<select id="tipoformulario" name="tipoformulario">
								<option value="inscripcion"
									<?php if($ret['tipo'] == 'inscripcion'){ echo 'selected="selected"';} ?>>Inscripción</option>
								<option value="evaluacion"
									<?php if($ret['tipo'] == 'evaluacion'){ echo 'selected="selected"';} ?>>Evaluación</option>
							</select>
						</div>
					</div>
						...
						<div class="col-md-7">
							<select class="form-control" style="width: 400px" id="categoria">
                               <?php
																															...
																															if (! empty ( $categorias ))
																																listarCat ( $categorias, 0, $ret ['categoria'] );
																															else
					...
					<div class="form-group wdatepicker">
						...
						<div class="col-md-4">
							<input id="fechainicio" name="fechainicio"
								value="<?php  echo $ret['fechainicio']; ?>" type="text"
					...
					<div class="form-group wdatepicker">
						...
						<div class="col-md-4">
							<input id="fechafin" name="fechafin"
								value="<?php  echo $ret['fechafin']; ?>" type="text"
								placeholder="" class="form-control  input-datepicker">
					...
					<div class="form-group wdatepicker">
						...
						<div class="col-md-4">
							<input id="fechacomienzocurso" name="fechacomienzocurso"
								value="<?php  echo $ret['fechacomienzocurso']; ?>" type="text"
								placeholder="" class="form-control  input-datepicker">
				  ...
				  <div style="display: none;" id="cargahoraria" class="form-group ">
						...
						<div class="col-md-4">
							<input name="cargahoraria" id="cargahoraria"
								value="<?php echo $ret['cargahoraria']?>" class="form-control"
								type="cargahoraria">
	...
	margin-right: 10px;
					...
					<div id="zonapago" style="display: none;">
								...
								<label class="chbox checkbox-inline">
									<div class="checker" id="uniform-sedebeabonar">
										<span><div class="checker" id="uniform-sedebeabonar">
												<span><input type="checkbox" id="sedebeabonar" value="1"
													<?php if($ret['abonar'] == '1'){ echo 'checked="checked"';} ?>></span>
											</div></span>
									</div>
								</label>
							</div>
						...
						<div style="display: none;" class="form-group sedebeabonar">
							...
							<div class="col-md-7">
								<input name="lugarabono"
									value="<?php  echo $ret['lugarabono']; ?>" id="lugarabono"
						...
						<div style="display: none;" class="form-group sedebeabonar">
							...
							<div class="col-md-7">
								...
								<select id="monedacostocurso" name="monedacostocurso"
									...
									<option value="$U"
										<?php if($ret['monedacostocurso'] == '$U'){ echo 'selected="selected"';} ?>>$U</option>
									<option value="U$S"
										<?php if($ret['monedacostocurso'] == 'U$S'){ echo 'selected="selected"';} ?>>U$S</option>
								</select> <input name="costocurso"
									value="<?php  echo $ret['costocurso']; ?>" id="costocurso"
						...
						<div style="display: none;"
							...
							<div class="col-md-4">
								<input id="fechaabonoinicio"
									value="<?php  echo $ret['fechaabonoinicio']; ?>"
									name="fechaabonoinicio" type="text" placeholder=""
						...
						<div style="display: none;"
							...
							<div class="col-md-4">
								<input id="fechaabonofin"
									value="<?php  echo $ret['fechaabonofin']; ?>"
									name="fechaabonofin" type="text" placeholder=""
									class="form-control  input-datepicker">
					...
					<div class="form-group" style="overflow: hidden; display: block;">
						...
						<div class="col-md-7">
							<textarea name="colderecha" id="colderecha" class="form-control"><?php  echo $ret['colderecha']; ?></textarea>
						</div>
					</div>
					<div class="form-group" style="overflow: hidden; display: block;">
						...
						<div class="col-md-7">
							<textarea name="colizquierda" id="colizquierda"
								class="form-control"><?php  echo $ret['colizquierda']; ?></textarea>
						</div>
					</div>
				...
				<div class=" stickycolumn"
					...
					<div id="formulario"
						class="droppedFields ui-droppable ui-sortable activeDroppable col-md-8">

      <?php

						$this->Admin_model->generarHTML ( $ret ['formulario'] );
						?>
...
<script type="text/javascript">
  $(function () {
          ...
          docReady();
              ...
              var seleccionado = '<?php echo $ret['vincular'];?>'

              $('select[name="vincular"]').css('width','400px').append('<option value="0">Ninguno</option>');
              $.each(formsInscripcion,function(index,value){

                var sel = '';
                if (formsInscripcion[index].id == seleccionado)
                  sel = 'selected';


                $('select[name="vincular"]').append('<option '+sel+' value="' + value.id+ '">'+value.titulo +'('+ value.fechainicio + ' - ' + value.fechafin +')'+'</option>');
					    ...
					    if (!valid) {
					      $(e.relatedTarget).addClass('invalid')
						   ...
						   $('#habilitarpersonascantidad').html( parseInt($('#habilitarpersonascantidad').text()) - 1);
								  ...
								  $('#habilitarpersonas').tokenfield('setTokens','<?php $i = 0;foreach($ret['emails'] as $email){$i++;echo $email;if ($i < count($ret['emails']))echo ';';}?>');
...
--->
<script id="control-customize-template"
